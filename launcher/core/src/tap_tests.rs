...
fn actions(recognizer: &mut TapRecognizer, events: &[KeyEvent]) -> Vec<Action> {
    events
        .iter()
        .filter_map(|event| recognizer.feed(*event))
        .collect()
}
...
fn recognizes_a_bare_left_meta_tap_within_400_ms() {
    ...
    assert_eq!(
        actions(
            &mut recognizer,
            &[key(LEFT_META, 1, 1000), key(LEFT_META, 0, 1400)]
        ),
        [Action::Tap]
    );
}
...
fn ignores_a_meta_press_held_longer_than_400_ms() {
    ...
    assert_eq!(
        actions(
            &mut recognizer,
            &[
                key(LEFT_META, 1, 1000),
                key(LEFT_META, 2, 1300),
                key(LEFT_META, 0, 1401)
            ]
        ),
        [],
    );
...
fn ignores_meta_used_in_a_key_or_button_chord() {
    ...
    let click = [
        key(LEFT_META, 1, 200),
        key(BUTTON_LEFT, 1, 250),
        key(BUTTON_LEFT, 0, 260),
        key(LEFT_META, 0, 300),
    ];
...
fn does_not_treat_right_meta_as_the_launcher_key() {
    ...
    assert_eq!(
        actions(
            &mut recognizer,
            &[key(RIGHT_META, 1, 0), key(RIGHT_META, 0, 100)]
        ),
        []
    );
}
...
fn meta_f12_toggles_the_guard_and_suppresses_taps_while_suspended() {
    ...
    let guard_chord = |start: u64| {
        [
            key(LEFT_META, 1, start),
            key(KEY_F12, 1, start + 50),
            key(KEY_F12, 0, start + 60),
            key(LEFT_META, 0, start + 100),
        ]
    };
    let tap = |start: u64| [key(LEFT_META, 1, start), key(LEFT_META, 0, start + 100)];
    assert_eq!(
        actions(&mut recognizer, &guard_chord(0)),
        [Action::GuardChanged { suspended: true }]
    );
    assert_eq!(actions(&mut recognizer, &tap(1000)), []);
    assert_eq!(
        actions(&mut recognizer, &guard_chord(2000)),
        [Action::GuardChanged { suspended: false }]
    );
...
fn starts_suspended_when_the_persisted_guard_says_so() {
    ...
    assert_eq!(
        actions(
            &mut recognizer,
            &[key(LEFT_META, 1, 0), key(LEFT_META, 0, 100)]
        ),
        []
    );
}
