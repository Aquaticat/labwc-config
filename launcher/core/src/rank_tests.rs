...
const NAMES: [&str; 6] = ["Font Viewer", "Firefox", "Barcode Scanner", "Code", "Files", "VS Code"];

/// Resolves search output back to names so assertions read as display order.
fn displayed(query: &str) -> Vec<&'static str> {
    search(query, &NAMES).into_iter().map(|index| NAMES[index]).collect()
}
...
fn lists_every_name_alphabetically_for_an_empty_query() {
    assert_eq!(displayed(""), ["Barcode Scanner", "Code", "Files", "Firefox", "Font Viewer", "VS Code"]);
}
