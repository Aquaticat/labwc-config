...
use super::{
    ProtocolError, Reply, Request, decode_reply, decode_request, encode_reply, encode_request,
};
...
fn round_trips_a_dmenu_request_with_prompt_and_lines() {
    let request = Request::Dmenu {
        prompt: "panel".into(),
        lines: vec!["Audio".into(), "Lock screen".into()],
    };
...
fn keeps_empty_dmenu_lines_but_not_the_final_terminator() {
    assert_eq!(
        ...
        Ok(Request::Dmenu {
            prompt: String::new(),
            lines: vec!["a".into(), String::new(), "b".into()]
        }),
    );
}
...
fn rejects_unknown_commands_and_invalid_utf8() {
    assert_eq!(
        decode_request(b"launch firefox\n"),
...
fn round_trips_selection_and_cancel_replies() {
    assert_eq!(
        encode_reply(&Reply::Selected("Lock screen".into())),
        b"Lock screen\n"
    );
    assert_eq!(
        decode_reply(b"Lock screen\n"),
...
#[test]
fn round_trips_a_close_request() {
